...
function writeLog(index, url, status, notes = '') {
    ...
    console.log(
        `${status === 'success' ? '✔' : '✖'} Logged: ${logEntry.trim()}`,
    );
}
...
async function processUrl(browser, url, index) {
    ...
    try {
        ...
        await page.screenshot({
            path: `${SCREENSHOTS_DIR}/${index}-desktop.png`,
        });
...
async function processBatch(batch, batchIndex, startIndex) {
    ...
    await Promise.all(
        batch.map((url, index) => {
            const absoluteIndex =
                batchIndex * MAX_CONCURRENT_BROWSERS + index + startIndex;
...
(async () => {
    ...
    const batches = Array.from(
        ...
        (_, i) =>
            remainingUrls.slice(
                i * MAX_CONCURRENT_BROWSERS,
                (i + 1) * MAX_CONCURRENT_BROWSERS,
            ),
    );
...
function notify(message) {
    ...
    if (platform === 'darwin') {
        exec(
            `osascript -e 'display notification "${message}" with title "Bookmarks"'`,
        );
